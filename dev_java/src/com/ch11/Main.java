...
class Game extends JFrame{  
    final static int WIDTH=GamePanel.Block.SIZE*GamePanel.NUM_OF_BLOCKS_HOR+30;  
    final static int HEIGHT=GamePanel.Block.SIZE*GamePanel.NUM_OF_BLOCKS_VER+50;  
...
class GamePanel extends JPanel{  
            ...
            while(isAlive) {    //살아있는 동안  
                while(!shape.isFallen()) {    //블럭이 떨어지지 않은 동안  
                    try {  
                        ...
                        //한칸 남았는데 내려와서 떨어졌든, 원래 떨어졌든 한 턴 더 줘  
                ...
                try {  
                     //java.util.ConcurrentModificationException의 임시방편  
                    sleep(10);  
                } catch (InterruptedException e) {  
                    // TODO Auto-generated catch block  
                    e.printStackTrace();  
    ...
    class Block{    //게임에서 블록의 클래스  
        ...
        //현재 block을 x+deltax,y+deltay로 이동. block이 존재하면 nothing  
        public void tryMove(int deltax, int deltay) {  
              
              
            //떨어져서 새로 만들지는 부 스레드가 결정, 얜 움직일지 말지만.  
            if(deltay!=0) {  
                if(!isFallen()) {  
                    y+=deltay;  
                }  
